...
export default function DashboardPage() {
  ...
  const cards = [
    ...
    { label: "Matriculas ativas", value: stats?.matriculas_ativas ?? 0, icon: ClipboardList },
    { label: "Frequencia media", value: `${stats?.media_presenca ?? 0}%`, icon: TrendingUp, color: "text-primary" },
  ];
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Painel de controle do projeto ELLP
        </p>
      </div>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Frequencia media</span>
                  ...
                  <div className="flex gap-2 pt-1">
                    <Link href={`/presenca?oficina=${oficina.id}`} className="flex-1">
                      <Button variant="outline" size="sm" className="w-full">Presenca</Button>
                    </Link>
                    <Link href={`/relatorios`} className="flex-1">
                      <Button variant="outline" size="sm" className="w-full">Relatorio</Button>
                    </Link>
                  </div>
                </CardContent>
              </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Como comecar</CardTitle>
        ...
        <CardContent className="space-y-3">
          ...
          <p className="text-sm text-muted-foreground">3. Realize as <strong>matriculas</strong> dos alunos nas oficinas</p>
          <p className="text-sm text-muted-foreground">4. Registre a <strong>presenca</strong> a cada aula</p>
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            <Link href="/oficinas">
              <Button>Nova oficina</Button>
            </Link>
            <Link href="/presenca">
              <Button variant="outline">Registrar presenca</Button>
            </Link>
          </div>
        </CardContent>
      </Card>
