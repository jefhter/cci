...
router.get("/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (result.rows.length === 0) {
      res.status(404).json({ error: "Oficina nao encontrada" });
      return;
    }
...
router.post("/", roleMiddleware("admin", "professor", "tutor"), async (req: Request, res: Response) => {
  ...
  if (!nome || nome.length < 2) {
    res.status(400).json({ error: "Nome e obrigatorio (minimo 2 caracteres)" });
    return;
  }
  ...
  try {
    ...
    res.status(500).json({ error: "Nao foi possivel criar a oficina" });
  } finally {
    client.release();
  }
});
...
router.put("/:id", roleMiddleware("admin", "professor", "tutor"), async (req: Request, res: Response) => {
  ...
  if (user.role === "professor") {
    ...
    if (check.rows.length === 0 || check.rows[0].professor_id !== user.userId) {
      res.status(403).json({ error: "Voce so pode editar suas proprias oficinas" });
      return;
    }
  }
  ...
  if (user.role === "tutor") {
    ...
    if (check.rows.length === 0) {
      res.status(403).json({ error: "Voce so pode editar oficinas das quais participa" });
      return;
    }
  }
  ...
  try {
    ...
    res.status(500).json({ error: "Erro ao salvar alteracoes da oficina" });
  } finally {
    client.release();
  }
});
...
router.delete("/:id", roleMiddleware("admin", "professor", "tutor"), async (req: Request, res: Response) => {
  ...
  if (user.role === "professor") {
    const check = await pool.query("SELECT professor_id FROM oficinas WHERE id = $1", [req.params.id]);
    if (check.rows.length === 0 || check.rows[0].professor_id !== user.userId) {
      res.status(403).json({ error: "Voce so pode remover suas proprias oficinas" });
      return;
    }
  }
  ...
  if (user.role === "tutor") {
    ...
    if (check.rows.length === 0) {
      res.status(403).json({ error: "Voce so pode remover oficinas das quais participa" });
      return;
    }
  }
