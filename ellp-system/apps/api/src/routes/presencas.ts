...
router.get("/", async (req: Request, res: Response) => {
  ...
  if (!aula_id) {
    res.status(400).json({ error: "aula_id e obrigatorio" });
    return;
  }
  ...
  try {
    ...
    res.status(500).json({ error: "Falha ao carregar presencas" });
  }
});
...
router.post("/toggle", async (req: Request, res: Response) => {
  ...
  if (!matricula_id || !aula_id || typeof presente !== "boolean") {
    res.status(400).json({ error: "matricula_id, aula_id e presente sao obrigatorios" });
    return;
  }
  ...
  try {
    ...
    res.status(500).json({ error: "Erro ao registrar presenca" });
  }
});
...
router.get("/stats", async (req: Request, res: Response) => {
  ...
  if (!oficina_id) {
    res.status(400).json({ error: "oficina_id e obrigatorio" });
    return;
  }
...
router.post("/recalcular", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({
      message: "Aprovacao recalculada com sucesso",
      aprovados,
      reprovados,
      total: stats.rows.length,
    });
  } catch (error) {
    console.error("Erro ao recalcular aprovacao:", error);
    res.status(500).json({ error: "Erro ao recalcular aprovacao" });
  }
});
