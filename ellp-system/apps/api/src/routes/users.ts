...
router.put("/:id/role", adminOnly, async (req: Request, res: Response) => {
  ...
  if (!role || !validRoles.includes(role)) {
    res.status(400).json({ error: "Papel invalido. Use: admin, professor ou tutor" });
    return;
  }

  if (currentUser.userId === targetId && role !== "admin") {
    res.status(400).json({ error: "Voce nao pode remover seu proprio papel de admin" });
    return;
  }
  ...
  try {
    ...
    res.status(500).json({ error: "Erro ao alterar papel do usuario" });
  }
});
