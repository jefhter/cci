...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: "Erro ao carregar matriculas" });
  }
});
...
router.post("/", async (req: Request, res: Response) => {
  ...
  if (!aluno_id || !oficina_id) {
    res.status(400).json({ error: "Aluno e oficina sao obrigatorios" });
    return;
  }
  ...
  try {
    ...
    if (existing.rows.length > 0) {
      res.status(409).json({ error: "Aluno ja esta matriculado nesta oficina" });
      return;
    }
    ...
    res.status(500).json({ error: "Nao foi possivel realizar a matricula" });
  }
});
...
router.delete("/:id", async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: "Erro ao excluir matricula" });
  }
});
